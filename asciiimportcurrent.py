...
file_path = r"C:\Users\dariu\Documents\Quail\fro.spk"
...
meshes, armature_data, track_definitions, material_palettes, include_files = eq_ascii_parse(file_path)
...
def create_mesh(mesh_data, parent_obj, armature_obj, cumulative_matrices):
    print(f"Creating mesh '{mesh_data['name']}'")
    print("Before mesh creation:")
    for face in mesh_data['faces']:
        print(f"Face vertices: {face[:3]}, Passable: {face[3]}")  # Assuming the 4th element is the "passable" flag
    ...
    for vg_start, vg_end, bone_index in mesh_data['vertex_groups']:
        ...
        for vert_index in range(vg_start, vg_end):
            ...
            print(f"Transformed Vertex {vert_index}: {vert.co}")
    ...
    for parent_index, child_indices in armature_data['relationships']:
        ...
        for child_index in child_indices:
            ...
            if child_bone:
                ...
                if child_bone.name == "NE":
                    ne_child = child_bone
                if first_child is None:
                    first_child = child_bone
                
                cumulative_matrices[child_bone.name] = child_bone.matrix

        if ne_child:
            parent_bone.tail = ne_child.head
        elif first_child:
            parent_bone.tail = first_child.head
...
# Create armature and link it to the main object
if armature_data:
