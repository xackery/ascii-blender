...
def process_track_definition(lines):
    track_def = {
        'name': '',
        'num_frames': 0,
        'frame_transforms': [],
...
def process_track_instance(lines):
    track_instance = {
        'name': '',
        'definition': '',
        'interpolate': False,
    ...
    for line in lines:
        if line.startswith("TAG"):
            track_instance['name'] = line.split('"')[1]
        elif line.startswith("DEFINITION"):
            track_instance['definition'] = line.split('"')[1]
...
def track_parse(sections):
    ...
    for instance in sections.get('TRACKDEFINITION', []):
        track_def = process_track_definition(instance)
        track_definitions[track_def['name']] = track_def

    for instance in sections.get('TRACKINSTANCE', []):
        track_instance = process_track_instance(instance)
